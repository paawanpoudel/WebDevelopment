...
riverplot_transcript <-function() {
	...
	createData2Plot_t <- function(genes, multiplier = 10, white.line.points = c(), white.space = 0.1)
	{
   		...
   		cell.type.expression$value <- exp(as.numeric(cell.type.expression$value))
        
        	## This is where I normalise the expression per proportion
		gene.expression <- de.genes[de.genes$ensembl_gene_id %in% unique(cell.type.expression$ensembl_gene_id), grepl("ensembl_gene_id|mean", colnames(de.genes))]
		colnames(gene.expression) <- sub("mean_([[:alpha:]]+)","\\1", colnames(gene.expression))
		gene.expression <- melt(gene.expression, variable_name = "cell.type", id = "ensembl_gene_id")
		gene.expression$value <- exp(as.numeric(gene.expression$value))
		colnames(gene.expression)[colnames(gene.expression) == "value"] = "sum.gene.exprs"
		cell.type.expression <- merge(gene.expression, cell.type.expression, by = c("ensembl_gene_id", "cell.type"))
        
		#   	## Sum the trans values per cell type
		sum.trans.values <- ddply(cell.type.expression, .(cell.type), summarize, sum.trans.exprs = sum(value))
		cell.type.expression <- merge(cell.type.expression, sum.trans.values, by="cell.type")
		## Calculate the proportion of trans per sum of trans
		cell.type.expression$proportions <- cell.type.expression$value/as.numeric(cell.type.expression$sum.trans.exprs)
		## This is where I normalise the expression per proportion
		cell.type.expression$value <- log2(cell.type.expression$sum.gene.exprs + 1)*cell.type.expression$proportions
	...
	index <- ((de.tx$ensembl_transcript_id %in% gene.sel) | (de.tx$ensembl_gene_id %in% gene.sel ) | (de.tx$hgnc_symbol %in% gene.sel))
